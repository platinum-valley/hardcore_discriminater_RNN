...
def inference(x):
    ...
    logits = tf.nn.softmax(logits)  # ソフトマックス
    return logits
...
def loss(logits, t):
    ...
    correct_prediction = tf.equal(tf.argmax(logits, 1), tf.argmax(t_on_hot, 1))
...
for train_idx, test_idx in k_fold.split(x_data, t_data):
    ...
    t = tf.placeholder(tf.int32, [None])  # 出力データ

    pred = inference(x)
    cross_entropy_loss, accuracy = loss(pred, t)
    train_step = training(cross_entropy_loss)
    # 学習の実行
    sess = tf.Session()
    ...
    for i in range(num_steps):
        ...
        if i % math.floor(num_data/batch_size) == 0:
            loss_, acc_ = sess.run([cross_entropy_loss, accuracy], feed_dict={x: x_train, t: t_train})
            loss_test_, acc_test_ = sess.run([cross_entropy_loss, accuracy], feed_dict={x: x_test, t: t_test})
